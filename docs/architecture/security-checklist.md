...
| **Rate-limit the HTML (non-DRF) views** | Brute force / credential stuffing / mass signup on `/accounts/login` + `/accounts/register` (DRF throttles don't reach plain Django views) | Before public launch | e.g. `django-axes` or `django-ratelimit`. The geocode-search proxy already has a basic cache throttle (per user when signed in, else per `REMOTE_ADDR`); this item is the robust, distributed version (shared cache) across all non-DRF endpoints. |
| **Proxy-aware client IP for anonymous throttling** | `/address-search/` is now public (customer discovery). Its anonymous throttle keys on `REMOTE_ADDR`, which behind a reverse proxy becomes the proxy's IP, collapsing all anonymous users into one bucket | Before public launch / when a proxy is in front | Configure trusted-proxy handling (e.g. `SECURE_PROXY_*` / a vetted `X-Forwarded-For` parser) so the throttle is per real client; pairs with the distributed throttle above. |
