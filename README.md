...
TruckStop is a two-sided platform that tracks food trucks in the local area. **Truck owners** sign up to promote their business, manage their profile, and post where they'll be and when. **Customers** discover trucks near them, follow the ones they love, and know exactly when and where to find them.
...
TruckStop is a **monorepo** with two codebases sharing one backend API:
...
TruckStop is a standalone product but lives in the shared project ecosystem (see `~/.claude/CLAUDE.md`). It reuses **design thinking and patterns** (not code) from existing projects:
